...
CSV_FIELDS = ['ts', 'reads', 'writes', 'fsyncs', 'response_time']

# The columns that can be plotted on the graphs
GRAPH_FIELDS = ['reads', 'writes', 'fsyncs', 'response_time']

# The labels of the co
GRAPH_FIELD_LABELS = {
    'reads': {'xlabel': 'time (s)', 'ylabel': 'Read MB/s', 'title': 'IO Read Bandwidth'},
    'writes': {'xlabel': 'time (s)', 'ylabel': 'Write MB/s', 'title': 'IO Write Bandwidth'},
    'fsyncs': {'xlabel': 'time (s)', 'ylabel': 'Fsyncs/second', 'title': 'IO Fsyncs'},
    'response_time': {'xlabel': 'time (s)', 'ylabel': 'P99 Latency (ms)', 'title': 'IO P99 Latency (ms)'}
}
...
@click.option('--plot-fields', '-c', default=['reads', 'writes', 'response_time'], multiple=True,
